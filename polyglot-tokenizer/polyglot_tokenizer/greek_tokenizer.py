...
class GreekTokenizer(RomanTokenizer):
    def __init__(self, lang='el', split_sen=False, smt=False):
        ...
        self.greek_alpha = ''.join(
            [unichr(x) for x in range(0x0370, 0x0400) if unichr(x).isalpha()])
        self.greek_alpha += ''.join([unichr(x)
                                     for x in range(0x1f00, 0x2000) if unichr(x).isalpha()])
        self.alpha += self.greek_alpha
        self.alpha_lower += ''.join(
            [x for x in self.greek_alpha if x.islower()])
        self.alpha_upper += ''.join(
            [x for x in self.greek_alpha if x.isupper()])
        # compile regexes
        self.fit()
        # recompile regexes
