...
def convert_encoding(filename, text_type, language, in_enc, out_enc):
    ...
    with codecs.open(filename, 'r', encoding='utf-8') as fp:
        ...
        if text_type == "ssf":
            ...
            if text_type == "tnt":
                converter = WXC(order='%s2%s' %
                                (in_enc, out_enc), lang=language, format_='tnt')
            elif text_type == "text":
                converter = WXC(order='%s2%s' %
                                (in_enc, out_enc), lang=language)
            elif text_type == "conll":
                converter = WXC(order='%s2%s' % (in_enc, out_enc),
                                lang=language, format_='conll')
            else:
...
def get_args():
    ...
    parser = argparse.ArgumentParser(
        description='Encoding Converter for Indian Languages')
...
if __name__ == "__main__":
    ...
    output_data = convert_encoding(
        args.input_path, args.data_format, args.language, args.in_enc, args.out_enc)
